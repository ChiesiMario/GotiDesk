<script lang="ts">
  ...
  onMount(() => {
    // Notify Rust backend that frontend is ready to be shown (prevents white flash)
...
{#if !isReady}
  <div class="h-screen w-screen flex flex-col items-center justify-center bg-[#111111] text-gray-400 text-sm selection:bg-white/20 relative">
    <div class="flex flex-col items-center justify-center animate-fade-in-up">
      <!-- We force the Logo into dark mode appearance by providing a context where text is white -->
      <div class="w-24 h-24 mb-6 text-white">
        <Logo />
      </div>
      
      {#if url}
        <div class="text-white/80 font-mono text-xs mb-8 tracking-wider">{url.replace(/^https?:\/\//, '').replace(/\/$/, '')}</div>
      {/if}
      ...
      <div class="flex items-center space-x-3 text-white/50">
        ...
        <span class="tracking-widest uppercase text-xs font-semibold">{t('common.loading')}</span>
      </div>
    </div>
  </div>
{:else}
              ...
              <div class="relative group w-full">
                ...
                <button 
                  ...
                  class={`absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-colors ${
                    activePopover?.id === 'global' ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus:opacity-100'
                  } ${
                    selectedAppId === null 
              ...
              {#each apps as app}
                <div class="relative group w-full">
                  ...
                  <button 
                    ...
                    class={`absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-md transition-colors ${
                      activePopover?.id === app.id.toString() ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus:opacity-100'
                    } ${
          ...
          <div class="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <div class="max-w-2xl w-full space-y-8 scroll-smooth">
              ...
              {:else if settingsActiveTab === 'about'}
                <div class="space-y-10 max-w-md mx-auto pb-10 pt-4 flex flex-col items-center text-center animate-fade-in-up">
